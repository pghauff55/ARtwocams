...
// Marker detection.
static ARHandle		*gARHandle1 = NULL;
...
static AR3DHandle	*gAR3DHandle1 = NULL;
...
static int			gPatt_id1;				// Per-marker, but we are using only 1 marker.
static int			gPatt_id2;

// Drawing.
static int gWindowW=640;
static int gWindowH=480;
...
static ARGL_CONTEXT_SETTINGS_REF gArglSettings1 = NULL;
...
char cparam_name[]="Data/cam_param.dat";

static int setupCameras()
{	
    ARParam			cparam;
	int				xsize, ysize;
    ...
    // Open the video path.
    if ( (gVid1=ar2VideoOpen("-dev=/dev/video0")) < 0) {
    	ARLOGe("setupCamera(): Unable to open connection to camera.\n");
    	return (FALSE);
	}
  if ( (gVid2=ar2VideoOpen("-dev=/dev/video1")) < 0) {
    	ARLOGe("setupCamera(): Unable to open connection to camera.\n");
    	return (FALSE);
	}
	
    // Find the size of the window.
   if (ar2VideoGetSize(gVid1,&xsize, &ysize) < 0) {
...
pixFormat=ar2VideoGetPixelFormat(gVid1);
	

        if (arParamLoad("Data/camera_para.dat", 1, &cparam) < 0) {
	printf("error loading camera parameter file............\n");
	}
    ...
    if ((gARHandle1 = arCreateHandle(gCparamLT1)) == NULL) {
        ARLOGe("setupCamera(): Error: arCreateHandle.\n");
        return (FALSE);
    }
    if (arSetPixelFormat(gARHandle1, pixFormat) < 0) {
        ARLOGe("setupCamera(): Error: arSetPixelFormat.\n");
        return (FALSE);
    }
	if (arSetDebugMode(gARHandle1, AR_DEBUG_DISABLE) < 0) {
        ARLOGe("setupCamera(): Error: arSetDebugMode.\n");
        return (FALSE);
    }

   if ((gCparamLT2 = arParamLTCreate(&cparam, AR_PARAM_LT_DEFAULT_OFFSET)) == NULL) {
        ARLOGe("setupCamera(): Error: arParamLTCreate.\n");
        return (FALSE);
    }

    if ((gARHandle2 = arCreateHandle(gCparamLT2)) == NULL) {
        ARLOGe("setupCamera(): Error: arCreateHandle.\n");
        return (FALSE);
    }
    if (arSetPixelFormat(gARHandle2, pixFormat) < 0) {
        ARLOGe("setupCamera(): Error: arSetPixelFormat.\n");
        return (FALSE);
    }
	if (arSetDebugMode(gARHandle2, AR_DEBUG_DISABLE) < 0) {
        ARLOGe("setupCamera(): Error: arSetDebugMode.\n");
        return (FALSE);
    }


	if ((gAR3DHandle1 = ar3DCreateHandle(&cparam)) == NULL) {
        ARLOGe("setupCamera(): Error: ar3DCreateHandle.\n");
        return (FALSE);
    }
	if ((gAR3DHandle2 = ar3DCreateHandle(&(gCparamLT2->param))) == NULL) {
        ARLOGe("setupCamera(): Error: ar3DCreateHandle.\n");
        return (FALSE);
    }
...
{
	arglCleanup(gArglSettings1);
arglCleanup(gArglSettings2);    
gArglSettings1 = NULL;
	arPattDetach(gARHandle1);
...
ar3DDeleteHandle(&gAR3DHandle1);
ar3DDeleteHandle(&gAR3DHandle2);

	arDeleteHandle(gARHandle1);
...
static void Keyboard(unsigned char key, int x, int y)
{
	...
	switch (key) {
		...
		case 'X':
		case 'x':
            arGetImageProcMode(gARHandle1, &mode);
            switch (mode) {
            ...
            arSetImageProcMode(gARHandle1, mode);
			break;
		case 'C':
		case 'c':
		...
		case 'a':
		case 'A':
			arGetLabelingThreshMode(gARHandle1, &modea);
            switch (modea) {
            ...
            arSetLabelingThreshMode(gARHandle1, modea);
			break;
		case '-':
			threshChange = -5;
			break;
		...
		case 'D':
		case 'd':
			arGetDebugMode(gARHandle1, &mode);
			arSetDebugMode(gARHandle1, !mode);
			break;
        case 's':
        case 'S':
	...
	if (threshChange) {
		int threshhold;
		arGetLabelingThresh(gARHandle1, &threshhold);
		threshhold += threshChange;
		if (threshhold < 0) threshhold = 0;
		if (threshhold > 255) threshhold = 255;
		arSetLabelingThresh(gARHandle1, threshhold);
	}
...
// Grab a video frame.
	image = ar2VideoGetImage(gVid2);

	if(image != NULL) {
        
        arglPixelBufferDataUpload(gArglSettings2, image->buff);
		}
	// Grab a video frame.
	image = ar2VideoGetImage(gVid1);

	if(image != NULL) {
        
        arglPixelBufferDataUpload(gArglSettings1, image->buff);
		...
		if (arDetectMarker(gARHandle1, image) < 0) {
			exit(-1);
		}
	markerNum = arGetMarkerNum(gARHandle1);
    if( markerNum == 0 ) {
  	//printf("marker num ==0");
        return;
    }
...
printf("(%d)",markerNum);
 markerInfo =  arGetMarker( gARHandle1 ); 
		k = -1;
		for (j = 0; j < markerNum; j++) {
printf("(%d)",markerInfo[j].id);
			if (markerInfo[j].id == gPatt_id1) {
...
printf("(%f,%f)",markerInfo[k].vertex[(4-dir)%4][0],markerInfo[k].vertex[(4-dir)%4][1]);
printf("(%f,%f)",markerInfo[k].vertex[(5-dir)%4][0],markerInfo[k].vertex[(5-dir)%4][1]);
printf("(%f,%f)",markerInfo[k].vertex[(6-dir)%4][0],markerInfo[k].vertex[(6-dir)%4][1]);
if(gAR3DHandle1->icpHandle!=NULL){
			printf("Handle is not NULL\n");
			// Get the transformation between the marker and the real camera into gPatt_trans.
			err = arGetTransMatSquareCont(gAR3DHandle1, &(markerInfo[k]),gPatt_trans, 78.0, gPatt_trans);
}
			gPatt_found = TRUE;
printf("Found Pattern.%f\n",err);
		} else {
			gPatt_found = FALSE;
		}
...
static void Display(void)
{
	...
	//glViewport(0, 0, (GLsizei) gWindowW, (GLsizei) gWindowH);

  glMatrixMode(GL_MODELVIEW);
    glPushMatrix();
   glLoadIdentity();
	arglDispImageStateful(gArglSettings1);
//glTranslatef(640.0,0.0,0.0);
//arglDispImageStateful(gArglSettings2);
   glPopMatrix();
...
int main(int argc, char** argv)
{
    ...
    char    patt_name2[] ="Data/patt1.dat";
 char    patt_name3[] ="Data/hiro.patt";
    //
  ...
  //  arUtilChangeToResourcesDirectory(AR_UTIL_RESOURCES_DIRECTORY_BEHAVIOR_BEST, NULL);
	...
	glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGBA | GLUT_DEPTH);
		...
		glutCreateWindow("twoCams 0.1----");
	

	// Setup ARgsub_lite library for current OpenGL context.
	if ((gArglSettings1 = arglSetupForCurrentContext(&(gCparamLT1->param), ar2VideoGetPixelFormat(gVid1))) == NULL) {
    ...
    arglSetupDebugMode(gArglSettings1, gARHandle1);
	arUtilTimerReset();
		
	// Load marker(s).
	if (!setupMarker(patt_name, &gPatt_id1, gARHandle1, &gARPattHandle)) {
		ARLOGe("main(): Unable to set up AR marker.\n");
		cleanup();
		exit(-1);
	}
		// Load marker(s).
	//if (!setupMarker(patt_name, &gPatt_id2, gARHandle2, &gARPattHandle)) {
	//	ARLOGe("main(): Unable to set up AR marker.\n");
	///	cleanup();
	//	exit(-1);
	//}
...
static void printMode()
{
    ...
    arGetLabelingThreshMode(gARHandle1, &threshMode);
    switch (threshMode) {
    ...
    if (threshMode != AR_LABELING_THRESH_MODE_AUTO_ADAPTIVE) {
        arGetLabelingThresh(gARHandle1, &thresh);
        len = (int)strlen(text);
    ...
    arGetBorderSize(gARHandle1, &tempF);
    snprintf(text, sizeof(text), "Border: %0.1f%%", tempF*100.0);
    arGetPatternDetectionMode(gARHandle1, &mode);
    switch (mode) {
